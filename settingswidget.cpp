...
SettingsWidget::SettingsWidget(QWidget *parent) : QWidget{parent}{
    ...
    main_layout->addWidget(new QLabel("Код початкової мови"));
    main_layout->addWidget(lang_source_key);
    main_layout->addStretch(1);
    main_layout->addWidget(new QLabel("Код мови перекладу"));
    main_layout->addWidget(lang_target_key);
    main_layout->addStretch(50);

    main_layout->addWidget(new QLabel("Сервер перекладу"));
    main_layout->addWidget(translate_address);
    main_layout->addStretch(200);

    auto_change_key->setText("Автоматично змінити ключ мови");
    auto_change_key->setChecked(true);
    main_layout->addWidget(auto_change_key);

    auto_change_file_name->setText("Автоматично змінити ключ мови у назві файлу");
